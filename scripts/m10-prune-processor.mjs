...
const proc = join(root, 'backend/src/modules/jobs/generate.processor.ts');
const text = readFileSync(proc, 'utf8');
const marker = '  private async handleShots(';
const idx = text.indexOf(marker);
if (idx < 0) throw new Error('handleShots marker not found');

const hs = text.indexOf('  private async handleScript(');
if (hs < 0) throw new Error('handleScript not found');
let methods = text.slice(hs, idx);
methods = methods.replace(
  /\n\s*\/\/ 写章 JSON 未带出场景时[\s\S]*?this\.logger\.warn\(`章节场景补提失败: \$\{e\?\.message \|\| e\}`\);\s*\}\s*\n/,
  '\n',
);

const catchStart = text.indexOf(
  '    } catch (e: any) {',
  text.indexOf('async run(jobRunId: string)'),
);
const catchEnd = text.indexOf('  private async handleScript(');
const origTail = text.slice(catchStart, catchEnd).trimEnd();

const runPrefix = `  async run(jobRunId: string) {
...
const newTop = `import { Inject, Injectable, Logger, forwardRef } from '@nestjs/common';
...
const out = `${newTop}${runPrefix}\n${origTail}\n\n${methods}${footer}`;
writeFileSync(proc, out, 'utf8');
console.log(`wrote ${proc} (${out.split('\\n').length} lines)`);
